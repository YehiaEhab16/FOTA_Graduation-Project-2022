...
typedef enum
{
  CAN_RXTX_STATE_RESET             = 0x00,  // CAN not yet initialized or disabled 
  CAN_RXTX_STATE_READY             = 0x01,  // CAN initialized and ready for use   
  CAN_RXTX_STATE_BUSY              = 0x02,  // CAN process is ongoing              
  CAN_RXTX_STATE_ERROR             = 0x04   // CAN error state                     

}CAN_RxTx_StateTypeDef;


typedef struct
{
  u32 Prescaler;  /* Specifies the length of a time quantum. 
                     This parameter must be a number between Min_Data = 1 and Max_Data = 1024. */
  
  u32 Mode;       // Specifies the CAN operating mode.
                   
  
  u32 SJW;        // Specifies the maximum number of time quanta 
                   
 
  u32 BS1;        // Specifies the number of time quanta in Bit Segment 1.
                  
  
  u32 BS2;        // Specifies the number of time quanta in Bit Segment 2.
                   
  
  u32 TTCM;       // Enable or disable the time triggered communication mode.
                    
  
  u32 ABOM;       // Enable or disable the automatic bus-off management.
                   
  
  u32 AWUM;       // Enable or disable the automatic wake-up mode. 
                   
  
  u32 NART;       // Enable or disable the non-automatic retransmission mode.
                   
  u32 RFLM;       // Enable or disable the Receive FIFO Locked mode.
                
  
  u32 TXFP;       // Enable or disable the transmit FIFO priority.
...
typedef struct
{
  ...
  CAN_StateTypeDef        State;    		  /*!< CAN communication state        */
 
  CAN_RxTx_StateTypeDef   rxState;      /*!< CAN communication state        */
 
  CAN_RxTx_StateTypeDef   txState;      /*!< CAN communication state        */
 
  u32                ErrorCode;  /*!< CAN Error code                 */
