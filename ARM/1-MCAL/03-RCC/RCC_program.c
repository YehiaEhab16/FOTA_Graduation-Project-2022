...
void RCC_voidInit (void)
{
	...
	RCC_CFGR&=~(7<<8);	/***************EDIT***********/
	RCC_CFGR|=(RCC_APB1_PRESCALER<<8);/***************EDIT***********/

	//Clear the prescaler bits of  APB2
	RCC_CFGR&=~(7<<11);/***************EDIT***********/
	RCC_CFGR|=(RCC_APB2_PRESCALER<<11);/***************EDIT***********/

	//Clear the prescaler bits of  AHB
	RCC_CFGR&=~(15<<4);/***************EDIT***********/
	RCC_CFGR|=(RCC_APB1_PRESCALER<<4);/***************EDIT***********/

	//Select the multiplication of PLL
	RCC_CFGR&=~(4<<18);/***************EDIT***********/
	RCC_CFGR|=(RCC_PLL_MULL<<18);/***************EDIT***********/
	...
	SET_BIT(RCC_AHBENR,RCC_SDIO-RCC_AHB_OFFSET);
	#endif

	#if RCC_FSMC_STATE		    ==    RCC_ENABLE
	SET_BIT(RCC_AHBENR,RCC_FSMC-RCC_AHB_OFFSET);
	#endif
	
	#if RCC_CRC_STATE		    ==    RCC_ENABLE
	SET_BIT(RCC_AHBENR,RCC_CRC-RCC_AHB_OFFSET);
	#endif
	
	#if RCC_FLITF_STATE		    ==    RCC_ENABLE
	SET_BIT(RCC_AHBENR,RCC_FLITF-RCC_AHB_OFFSET);
	#endif
	
	#if RCC_SRAM_STATE		    ==    RCC_ENABLE
	SET_BIT(RCC_AHBENR,RCC_SRAM-RCC_AHB_OFFSET);
	#endif
	
	#if RCC_DMA2_STATE		    ==    RCC_ENABLE
	SET_BIT(RCC_AHBENR,RCC_DMA2-RCC_AHB_OFFSET);
	#endif
	
	#if RCC_DMA1_STATE		    ==    RCC_ENABLE
	SET_BIT(RCC_AHBENR,RCC_DMA1-RCC_AHB_OFFSET);
	#endif
	
	#if RCC_TIM11_STATE	    ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_TIM11-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_TIM10_STATE	    ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_TIM10-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_TIM9_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_TIM9-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_ADC3_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_ADC3-RCC_APB2_OFFSET);	
	#endif
	
	#if RCC_USART1_STATE	    ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_USART1-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_TIM8_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_TIM8-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_SPI1_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_SPI1-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_TIM1_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_TIM1-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_ADC2_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_ADC2-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_ADC1_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_ADC1-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPG_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPG-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPF_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPF-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPE_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPE-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPD_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPD-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPC_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPC-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPB_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPB-RCC_APB2_OFFSET);
	#endif
	
	#if RCC_IOPA_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_IOPA-RCC_APB2_OFFSET);
	#endif
	
	#if   RCC_AFIO_STATE	        ==     RCC_ENABLE
	SET_BIT(RCC_APB2ENR,RCC_AFIO-RCC_APB2_OFFSET);
	#endif
	
	#if   RCC_DAC_STATE			==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_DAC	-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_PWR_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_PWR	-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_BKP_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_BKP	-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_CAN_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_CAN	-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_USB_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_USB	-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_I2C1_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_I2C1-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_I2C2_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_I2C2-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_UART5_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_UART5-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_UART4_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_UART4-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_USART3_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_USART3-RCC_APB1_OFFSET);
	#endif
	#if   RCC_USART2_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_USART2-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_SPI3_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_SPI3-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_SPI2_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_SPI2-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_WWDG_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_WWDG-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM14_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM14-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM13_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM13-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM12_STATE	    ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM12-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM7_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM7-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM6_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM6-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM5_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM5-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM4_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM4-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM3_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM3-RCC_APB1_OFFSET);
	#endif
	
	#if   RCC_TIM2_STATE	        ==    RCC_ENABLE
	SET_BIT(RCC_APB1ENR,RCC_TIM2-RCC_APB1_OFFSET);
	#endif
}
...
u8 RCC_u8DisableClock(u8 Copy_u8PeripheralID)
{
	...
	if(Copy_u8PeripheralID <=AHB_END  && Copy_u8PeripheralID >=AHB_START)
		CLR_BIT(RCC_AHBENR,(Copy_u8PeripheralID-RCC_AHB_OFFSET));

	else if(Copy_u8PeripheralID <=APB1_END  && Copy_u8PeripheralID >=APB1_START)
		CLR_BIT(RCC_APB1ENR,(Copy_u8PeripheralID-RCC_APB1_OFFSET));
	
	else if (Copy_u8PeripheralID <=APB2_END  && Copy_u8PeripheralID >=APB2_START)
		CLR_BIT(RCC_APB2ENR,(Copy_u8PeripheralID-RCC_APB2_OFFSET));
